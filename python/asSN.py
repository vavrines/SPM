...
def plotresult(yedges, xedges, psi0, psi, qweights, style):
    plt.style.use(
        "https://raw.githubusercontent.com/camminady/kitstyle/master/kitishnotex.mplstyle"
    )
    ...
    for data, ax, title in zip([phi0, phi], axs, ["Initial", "Final"]):
        ...
        ax.set_yticks([yedges[0], yedges[-1]])
        ax.set_xticks([xedges[0], xedges[-1]])
...
def euler(
    ...
    for i in numba.prange(psi.shape[1]):
        for j in range(psi.shape[2]):
            scattering[:, i, j] = (
                sigmas[i, j] * kernelphysical + sigmaas[i, j] * kernelartificial
            ) @ psi[:, i, j]
    # transport equation
...
def run(psi, dt, dx, dy, nt, qpoints, qweights, sigmas, sigmaa, sigmaas, beta, source):
    ...
    scatterartificial = computescattering(
        artificialkernel, beta / nq, qpoints, qweights
    )

    for timestep in range(nt):
        print("Iteration:", timestep, "of", nt - 1)
        psiOld = np.copy(psi)
...
def execute(problemparams, numericparams):
    ...
    start = time.time()
    psi = run(
        psi, dt, dx, dy, nt, qpoints, qweights, sigmas, sigmaa, sigmaas, beta, source
    )
...
def sigmaa(y, x):
    if (
        ((1 <= y <= 2 or 5 <= y <= 6) and (1 <= x <= 2 or 3 <= x <= 4 or 5 <= x <= 6))
...
def initparams(NX = 100, NY = 100, NQ = 3, CFL = 0.95):
    ny, nx, cfl, quaddegree = NY, NX, CFL, NQ
...
def initsol(problemparams, numericparams):
    y0, y1, x0, x1, t, fsigmas, fsigmaa, fpsi0, fsource, style = problemparams
    ny, nx, cfl, qpoints, qweights, fsigmaas, beta = numericparams
    nq = len(qweights)
